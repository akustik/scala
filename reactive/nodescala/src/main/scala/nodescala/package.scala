...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      new CancellableTokenSubscription(src)
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      try {
        ...
        case t: TimeoutException => throw new NoSuchElementException("Element not available")
      }
    }
  ...
  class CancellableTokenSubscription(src: CancellationTokenSource) extends Subscription {
    def unsubscribe(): Unit = src.unsubscribe
  }

...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    var request = listener.nextRequest
    val responseSubscription =
      Future.run() { ct =>
        {
          Future {
            while (ct.nonCancelled) {
              blocking {
                try {
                  val (req, exch) = Await.result(request, 1 second)
                  request = listener.nextRequest
                  async {
    ...
    Subscription(listenerSubscription, responseSubscription)
  }
}
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      createContext(handler => {
        val req = handler.request
        p.success((req, handler))
        removeContext
      })
      p.future
    }

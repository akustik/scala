...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def mostRetweeted: Tweet = mostRetweetedCandidate(new Tweet("", "", -1))
...
class Empty extends TweetSet {
  ...
  def mostRetweetedCandidate(candidate: Tweet): Tweet =
    if (candidate.retweets == -1) throw new NoSuchElementException("Empty.mostRetweeted")
    else candidate

  def descendingByRetweetAcc(t: TweetList): TweetList = t
...
trait TweetList {
  ...
  def revert: TweetList = revertAcc(Nil)
  def revertAcc(t: TweetList): TweetList = {
    if (!isEmpty) {
      tail.revertAcc(new Cons(head, t))
    } else {
      t
    }
  }
